...
def segment(result, config):
    ...
    frames = get_frames(result, config)

    segments = get_segments(frames, config)

    utterances = align_labels_to_segments(segments, result)
...
def get_segments(frames, config):
    ...
    for frame in frames:
        ...
        logger.debug("Segment byte length: " + str(len(data)) + ", frame ms: " + str(len(frame.segment)) + ", frame rate: " + str(frame.sample_rate) + " data: " + str(data[0:10]) )
